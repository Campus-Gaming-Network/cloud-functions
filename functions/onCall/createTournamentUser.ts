...
exports.createTournamentParticipant = functions.https.onCall(
  async (data, context) => {
    if (!data || !context) {
    ...
    const participantEmail =
      data.participantType === "user" ? context.auth.token.email : null;
    ...
    try {
      await tournamentUserDocRef.set(
        { id: tournamentUserDocRef.id },
        { merge: true }
      );
    } catch (error: any) {
      throw new functions.https.HttpsError(error.code, error.message);
